...
import { Box, Tabs, Tab, Fab, Tooltip, Badge } from "@mui/material";
...
const ExcelUploaderStorage = () => {
  ...
  const importFromDisk = (e) => {
    ...
    const expectedPrefix = `${activeSector}-${activeGrupo}-${activeCod}`;
    if (!file) return;
    if (!file.name.startsWith(expectedPrefix)) {
      toast.error(`Nombre inválido. Debe empezar por: ${expectedPrefix}`);
      return;
    }
  ...
  const exportToExcelFromSession = () => {
    ...
    const fileName = `${activeSector}-${activeGrupo}-${activeCod}-${dateStr}.xlsx`;
    XLSX.writeFile(workbook, fileName);
    toast.success("Generando archivo de descarga...");
  };
  ...
  return (
    <Box sx={{ width: "100%", bgcolor: "#fafafa", minHeight: "100vh" }}>
      <Box sx={{ maxWidth: "1400px", margin: "0 auto", p: 2, display: "flex", flexDirection: "column" }}>
        ...
        {/* TABS DE HOJAS CENTRADOS */}
        {/* CONTENEDOR FIJO DE TABS */}
        <Box sx={{
          position: "fixed",
          top: 35, // Si tienes un Navbar arriba, cámbialo a la altura del Navbar (ej: "64px")
          left: 0,
          right: 0,
          bgcolor: "white",
          zIndex: 1100,
          ...
          // Añadimos un pequeño padding lateral para que las flechas no se peguen al borde
          px: { xs: 1, sm: 2 }
        }}>
          <Box sx={{ width: "100%", maxWidth: "1400px" }}>
            <Tabs
              ...
              sx={{
                minHeight: '36px', // Altura total de la barra más ajustada
                height: '36px',
                '& .MuiTabs-scrollButtons': {
                  height: '36px', // Alineamos las flechas a la nueva altura
              ...
              {sheetNames.map((name, i) => (
                <Tab
                  ...
                  sx={{
                    ...
                    fontSize: '0.75rem',
                    minWidth: 'auto',
                    minHeight: '36px', // Debe coincidir con el contenedor
                    paddingTop: '4px',    // Mantiene un pequeño aire arriba
                    paddingBottom: '4px', // Acerca el texto a la línea de abajo
                    paddingLeft: '12px',
                    paddingRight: '12px',
                    lineHeight: 1,       // Reduce el espacio entre líneas del texto
                ...
                activeSheetData.map((row, r) => (
                  <Box key={r} sx={{ display: "flex", alignItems: "center", borderBottom: "1px solid #eee" }}>
                    {(row || []).map((cell, c) => {
                      ...
                      // 1. LÓGICA DE DISTRIBUCIÓN [x,y,z]
                      ...
                      // 2. DETERMINAR SI LA CELDA ES UN SELECTOR
                      if (isSelector(cell)) {
      ...
      {!ActiveComponent && currentSheetName && (
        <Tooltip title={`Guardar cambios en ${currentSheetName}`}>
          <Badge
            ...
            sx={{
              position: "fixed", // Posicionamiento independiente
              top: 70,
              left: '70%',
              zIndex: 2000,
      ...
      <Tooltip title="Importar Excel">
        <Fab
          ...
          sx={{
            position: "fixed", // Posicionamiento independiente
            top: 70,
            left: '80%', // Ajusta este valor para moverlo horizontalmente
            zIndex: 2000,
      ...
      <Tooltip title="Exportar Excel">
        <Fab
          onClick={exportToExcelFromSession}
          sx={{
            position: "fixed", // Posicionamiento independiente
            top: 70,
            left: '90%', // Ajusta este valor para moverlo horizontalmente
            zIndex: 2000,
            bgcolor: "success.main",
            color: "white",
            width: 32,
