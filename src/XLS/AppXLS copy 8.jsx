...
const ExcelUploaderStorage = () => {
  ...
  const duplicateRow = (sheet, rowIndex) => {
    setExcelData(prev => {
      ...
      newData.splice(rowIndex + 1, 0, rowToCopy); // Inserta la copia debajo
  ...
  const deleteRow = (sheet, rowIndex) => {
    setExcelData(prev => {
      ...
      // Si los datos de la hoja son undefined o tienen 1 o menos filas, BLOQUEAR
      if (!currentSheetData || currentSheetData.length <= 1) {
        console.warn("Bloqueo de borrado: Solo queda una fila.");
        return prev;
      }

      // Clonamos el objeto de hojas y la hoja específica
    ...
    <Box sx={{ width: "100%", bgcolor: "#fafafa", minHeight: "100vh" }}>
        ...
        {CONFIG.SHEET_COMPONENTS[currentSheet] ? (
          ...
          <Box sx={{
            ...
            <Box sx={{ display: "table", margin: "0 auto", minWidth: "100%" }}>
              {/* Dentro del map de filas (r) */}
              {(excelData[currentSheet] || []).map((row, r) => {
                const isHeader = r === 0;
                const isUpper = Utils.isUpperSheet(currentSheet);
                const totalRowsInSheet = excelData[currentSheet].length;
                      ...
                      <Box sx={{
                        ...
                        {!isHeader && (
                          <>
                            ...
                            {/* SOLO RENDERIZA EL BOTÓN SI HAY MÁS DE UNA FILA */}
                            {totalRowsInSheet > 1 ? (
                              ...
                              /* Espaciador invisible para que no se mueva la tabla al desaparecer el icono */
                              <Box sx={{ width: 26 }} />
                            )}
                          </>
                        )}
      ...
      <ProjectFormDialog
        ...
        onSaveAll={(newPdfData) => {
          // 1. Obtenemos lo que hay actualmente en sesión (que es la "verdad absoluta")
          const sessionRaw = sessionStorage.getItem("excelData");
          let currentDataInSession = excelData;

          if (sessionRaw) {
            try {
              currentDataInSession = JSON.parse(sessionRaw);
            } catch (e) {
              console.error("Error parseando sesión", e);
            }
          }

          // 2. Creamos el nuevo objeto mezclando lo de sesión con el nuevo PDF
          const updatedFullData = {
            ...currentDataInSession,
            PDF: newPdfData
          };

          // 3. Sincronizamos estado de React y SessionStorage
          setExcelData(updatedFullData);
          broadcastAndSave(updatedFullData);
          toast.success("Configuración de proyecto actualizada");
        }}
      />
