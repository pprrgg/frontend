        ...
        const data = parsed["Diseño_FV"]
          ...
          .map((r) => {
            ...
            return {
              ...
              rotation:
                Number.isFinite(parseFloat(row.rotation))
                  ? parseFloat(row.rotation)
                  : 0,
              ...
              config: {
                ...
                tipoEstructura:
                  row.tipoEstructura || "coplanar",

                width:
                  Number.isFinite(parseFloat(row.width))
                    ? parseFloat(row.width)
                    : 1.1,

                height:
                  Number.isFinite(parseFloat(row.height))
                    ? parseFloat(row.height)
                    : 1.9,

                tilt:
                  Number.isFinite(parseFloat(row.tilt))
                    ? parseFloat(row.tilt)
                    : 30,

                slope:
                  Number.isFinite(parseFloat(row.slope))
                ...
                potenciaW:
                  Number.isFinite(parseFloat(row.potenciaW))
                    ? parseFloat(row.potenciaW)
                    : 450,

                rows:
                  Number.isFinite(parseInt(row.rows))
                    ? parseInt(row.rows)
                    : 1,

                cols:
                  Number.isFinite(parseInt(row.cols))
                    ? parseInt(row.cols)
                    : 1
              ...
              paneles:
                Array.isArray(row.paneles)
                  ? row.paneles
                  : [],

              sombras:
                Array.isArray(row.sombras)
                  ? row.sombras
                  : Array(16).fill(1)
  ...
  const handleSave = useCallback(() => {
    ...
    try {
      ...
      const rowsData = grids.map((g) => [
        ...
        // NUEVOS CAMPOS CONSTANTES
        "FV",
        "Paneles",

        g.name,

        g.config?.tipoEstructura || "coplanar",
