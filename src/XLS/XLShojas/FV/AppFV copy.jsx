...
import {
  ...
  DialogTitle, Divider, Slider, Tab, Tabs, Tooltip, Badge, Fab, Button
} from "@mui/material";
import {
    ...
    for (let r = minR; r <= maxR; r++) {
      ...
      for (let c = minC; c <= maxC; c++) {
        ...
        if (isActive) {
          ...
          cell.on('click', () => this.onSelect(grid.id));
        }
      }
    }
  }
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e) => {
      if (e.key === "excelData") loadDataFromSession(true);
    };
  ...
  const handleSave = useCallback(() => {
    ...
    sessionStorage.setItem("excelData", finalStr);
  ...
  return (
    <Box sx={{ width: "100%", height: "100vh", position: "relative" }}>
      ...
      <Box sx={{ position: "absolute", top: 15, left: 15, width: 220, zIndex: 1000 }}>
        <Paper elevation={4} sx={{ borderRadius: 2, overflow: 'hidden', border: '1px solid #1a237e' }}>
          <Box sx={{ p: 1.5, display: 'flex', flexDirection: 'column', alignItems: 'center', bgcolor: '#1a237e', color: 'white', gap: 1 }}>
            ...
            <Stack direction="row" spacing={1}>
              <IconButton size="small" sx={{ color: 'white', border: '1px solid rgba(255,255,255,0.2)' }} onClick={() => setActiveGridId(null)}>
                {activeGridId ? <CollapseIcon fontSize="small" /> : <ExpandAllIcon fontSize="small" />}
              </IconButton>
              ...
              <IconButton
                size="small"
                sx={{ color: "white", border: '1px solid rgba(255,255,255,0.4)', bgcolor: 'rgba(255,255,255,0.1)' }}
                onClick={() => {
          ...
          {grids.length > 0 && (
            <Box sx={{ maxHeight: '75vh', overflowY: 'auto', bgcolor: 'white' }}>
              {grids.map((g, index) => {
                const isExpanded = activeGridId === g.id;
                ...
                return (
                  <Accordion
                    key={g.id}
                    expanded={isExpanded}
                    sx={{ '&:before': { display: 'none' }, m: '0 !important', borderBottom: '1px solid #eee' }}
                  >
                    <AccordionSummary
                      sx={{ px: 1, '& .MuiAccordionSummary-content': { alignItems: 'center', my: 1, overflow: 'hidden' } }}
                      onClick={() => {
                        if (!isExpanded) setActiveGridId(g.id);
                      }}
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}>
                        ...
                        <Stack>
                          <IconButton
                            size="small"
                            disabled={index === 0}
                            onClick={(e) => { e.stopPropagation(); moveGrid(index, -1); }}
                            sx={{ p: 0, height: 14 }}
                          >
                        ...
                        {isEditingName === g.id ? (
                          <TextField
                            autoFocus size="small" variant="standard" value={g.name}
                            onClick={(e) => e.stopPropagation()}
                            onBlur={() => setIsEditingName(null)}
                            onKeyDown={(e) => e.key === 'Enter' && setIsEditingName(null)}
                            onChange={(e) => updateGrid(g.id, { name: e.target.value })}
                            sx={{ width: '50%' }}
                          />
                        ) : (
                          <Box onDoubleClick={(e) => { e.stopPropagation(); setIsEditingName(g.id); }} sx={{ flexGrow: 1, overflow: 'hidden' }}>
                            <Typography variant="caption" fontWeight={900} sx={{ display: 'block', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden', lineHeight: 1.1 }}>
                              {g.name}
                            </Typography>
                            <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.65rem' }}>
                              {areaPwp}kWp · {g.paneles?.length || 0}u.
                            </Typography>
                          </Box>
                        )}
                        ...
                        <IconButton
                          ...
                          onClick={(e) => {
                            ...
                            if (isExpanded) setActiveGridId(null);
                          }}
                        >
                        ...
                        <Box sx={{ flexGrow: 1, p: 1.5, overflow: 'hidden' }}>
                          {innerTab === 0 ? (
                            <Stack spacing={2} alignItems="center">
                              <IconButton
                                onClick={() => updateGrid(g.id, { config: { orientation: g.config.orientation === 'vertical' ? 'horizontal' : 'vertical' } })}
                                sx={{ border: '1px solid #ddd', bgcolor: '#f5f5f5' }}
                              >
                                {g.config.orientation === 'vertical' ? <PortraitIcon /> : <LandscapeIcon />}
                              </IconButton>
                              <TextField label="Wp" size="small" variant="standard" fullWidth type="number" value={g.config.potenciaW} onChange={(e) => updateGrid(g.id, { config: { potenciaW: parseFloat(e.target.value) || 0 } })} />
                              <TextField label="H (m)" size="small" variant="standard" fullWidth value={g.config.height} onChange={(e) => updateGrid(g.id, { config: { height: parseFloat(e.target.value) || 0 } })} />
                              <TextField label="W (m)" size="small" variant="standard" fullWidth value={g.config.width} onChange={(e) => updateGrid(g.id, { config: { width: parseFloat(e.target.value) || 0 } })} />
                            </Stack>
                          ) : (
                            <Stack spacing={2} alignItems="center">
                              <Box sx={{ width: '100%', textAlign: 'center' }}>
                                <AzimutPreview rotation={g.rotation} />
                                <Typography variant="caption" display="block" fontWeight={700}>Azim: {Math.round(g.rotation)}°</Typography>
                                <Slider size="small" value={g.rotation} min={-180} max={180} onChange={(_, v) => updateGrid(g.id, { rotation: v })} />
                              </Box>
                              <Stack spacing={0.5} width="100%">
                                {['coplanar', 'libre', 'doble'].map((tipo) => (
                                  <Button
                                    ...
                                    sx={{ fontSize: '0.6rem', py: 0.3, textTransform: 'capitalize' }}
                                    onClick={() => {
                              ...
                              <Box sx={{ width: '100%', textAlign: 'center', mt: -2 }}>
                                ...
                                <Typography variant="caption" display="block" fontWeight={700}>Inc: {g.config.tilt}°</Typography>
                                <Slider size="small" value={g.config.tilt} min={0} max={90} disabled={g.config.tipoEstructura === 'coplanar'} onChange={(_, v) => {
                                  const val = g.config.tipoEstructura === 'libre' ? Math.max(v, g.config.slope) : v;
                                  updateGrid(g.id, { config: { tilt: val } });
                                }} sx={{ mb: 1 }} />
                                <Typography variant="caption" display="block" fontWeight={700} color="orange">Suelo: {g.config.slope}°</Typography>
                                <Slider size="small" value={g.config.slope} min={0} max={90} sx={{ color: 'orange' }} disabled={g.config.tipoEstructura === 'doble'} onChange={(_, v) => {
                                  let updates = { slope: v };
                              ...
                              <IconButton size="small" onClick={() => setShadowModalOpen(true)} sx={{ border: "1px solid #ddd" }}>
      ...
      {/* --- 4. EL BOTÓN FLOTANTE (CÓPIALO JUSTO AQUÍ) --- */}
      {/* BOTÓN FLOTANTE ESTILO FAB MINI CON COLORES EXACTOS */}
      <Box sx={{ position: "fixed", top: 100, right: 100, zIndex: 9999 }}>
        <Tooltip title={hasPendingChanges ? "Guardar cambios" : "Diseño guardado"} placement="left">
          <Badge
            color="error"
            variant="dot"
          ...
          >
            <Fab
              onClick={handleSave}
              sx={{
                // Color de fondo azul vibrante como en la imagen
                bgcolor: "#1976d2",
                // Color del icono blanco como en la imagen
                color: "#ffffff",
                width: 32, // Tamaño mini solicitado
                height: 32,
                minHeight: 32,
                "&:hover": {
            ...
            >
              {/* Asegúrate de que el icono SaveIcon esté importado */}
