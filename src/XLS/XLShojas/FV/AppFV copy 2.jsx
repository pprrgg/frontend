...
const App = () => {
  ...
  return (
    <Box sx={{ width: "100%", height: "100vh", position: "relative", overflow: 'hidden' }}>
      ...
      {/* MENU LATERAL */}
      <Box sx={{
        position: "absolute", top: 5, left: 15, width: 220, zIndex: 1000,
        ...
        <Paper elevation={4} sx={{ borderRadius: 2, overflow: 'hidden', border: '1px solid #1a237e' }}>
          <Box sx={{ p: 1.5, display: 'flex', flexDirection: 'column', alignItems: 'center', bgcolor: '#1a237e', color: 'white', gap: 1 }}>
            <Stack direction="row" spacing={1} alignItems="baseline">
              <Typography variant="subtitle2" fontWeight={900}>{(totals.power / 1000).toFixed(2)} kWp</Typography>
              <Typography variant="caption" sx={{ opacity: 0.8 }}>({totals.panels} mod)</Typography>
            </Stack>
            <IconButton size="small" sx={{ color: 'white', border: '1px solid rgba(255,255,255,0.2)' }} onClick={() => setMenuVisible(false)}>
                        ...
                        <Box sx={{ flexGrow: 1, p: 1.5, overflow: 'hidden' }}>
                          {innerTab === 0 ? (
                            <Stack spacing={2} alignItems="center">
                              <IconButton onClick={() => updateGrid(g.id, { config: { orientation: g.config.orientation === 'vertical' ? 'horizontal' : 'vertical' } })} sx={{ border: '1px solid #ddd', bgcolor: '#f5f5f5' }}>
                            ...
                            <Stack spacing={2} alignItems="center">
                              ...
                              <Stack spacing={0.5} width="100%">
                                {['coplanar', 'libre', 'doble'].map((tipo) => (
                                  <Button key={tipo} size="small" variant={g.config.tipoEstructura === tipo ? "contained" : "outlined"} sx={{ fontSize: '0.6rem', py: 0.3, textTransform: 'capitalize' }} onClick={() => {
                              ...
                              <IconButton size="small" onClick={() => setShadowModalOpen(true)} sx={{ border: "1px solid #ddd" }}><ShadowIcon fontSize="small" /></IconButton>
                            </Stack>
                          )}
                        </Box>
                      </Stack>
      ...
      {/* BOTON PARA REABRIR MENU */}
      {!menuVisible && (
        <Fab size="small" onClick={() => setMenuVisible(true)} sx={{ position: 'absolute', top: 15, left: 15, zIndex: 1000, bgcolor: '#1a237e', color: 'white' }}>
          <ExpandAllIcon />
        </Fab>
      )}

      {/* BOTÓN FLOTANTE AGREGAR */}
      <Box sx={{ position: "fixed", top: 100, left: "45%", zIndex: 9999 }}>
        <Tooltip title="Añadir paneles" placement="left">
          <Fab
