          ...
          .map((r) => {
            ...
            return {
              ...
              config: {
                ...
                // --- RECUPERACIÓN DE DATOS DEL INVERSOR ---
                ...
                vMin: Number.isFinite(parseFloat(row.vMin)) ? parseFloat(row.vMin) : 0,
  ...
  const handleSave = useCallback(() => {
    ...
    try {
      ...
      const rowsData = grids.map((g) => [
        ...
        g.config?.cols || 1,
        // --- VALORES INVERSOR ---
        g.config?.inverterModel || "",
        g.config?.inverterPower || 0,
        g.config?.mpptCount || 1,
        g.config?.vMin || 0,
        g.config?.vMax || 0,
  ...
  const handleAddNew = useCallback(() => {
    ...
    const n = {
      ...
      config: {
        ...
        rows: 1,
        cols: 1,
        // --- NUEVAS PROPIEDADES INVERSOR POR DEFECTO ---
        inverterModel: "",
        inverterPower: 0,
        mpptCount: 1,
        vMin: 0,
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            ...
            top: 150,
            m: 0,
            width: 240, // Incrementado ligeramente a 240 para dar más espacio a los formularios
            maxHeight: '80vh',
            borderRadius: 2,
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', bgcolor: 'white' }}>
            ...
            <Box sx={{ flexGrow: 1, p: 1.5, overflow: 'hidden' }}>
              {innerTab === 0 ? (
                ...
                /* PESTAÑA 3: CONFIGURACIÓN DE INVERSOR (NUEVA SOLAPA) */
                <Stack spacing={2}>
                  <Box sx={{
                    width: '100%',
                    bgcolor: '#f1f3f9',
                  ...
                  <Box sx={{ width: '100%', p: 0.5 }}>
                    ...
                    <Divider sx={{ width: '100%', my: 1, fontSize: '0.6rem', color: '#999' }}>RANGO TENSIONES MPPT</Divider>
                    ...
                    <Stack direction="row" spacing={1} width="100%">
                      <TextField
                        ...
                        type="number"
                        fullWidth
                        value={activeGrid.config.vMin || ''}
                        onChange={(e) => updateGrid(activeGridId, { config: { vMin: parseFloat(e.target.value) || 0 } })}
                        inputProps={{ style: { fontSize: '0.8rem', padding: '8px' } }}
                      />
                      <TextField
                        label="Vmax (V)"
