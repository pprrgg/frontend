  ...
  for (let r = minR; r <= maxR; r++) {
    ...
    for (let c = minC; c <= maxC; c++) {
      ...
      const cell = L.polygon(corners, {
        ...
        // 🔑 CAMBIO CLAVE: Solo es interactivo si es el área activa
      ...
      if (active) {
        L.marker(cellLatLng, {
          icon: L.divIcon({
            className: 'arrow',
            html: `<div style="transform: rotate(${arrowRad}rad); display: flex; justify-content: center; align-items: center;">${arrowWhiteSvg}</div>`,
            iconSize: [14, 14],
            iconAnchor: [7, 7]
          }),
          interactive: false
        ...
        L.marker(cellLatLng, {
          icon: L.divIcon({
            className: 'grid-plus',
            html: `<div style="color: #ffffff; font-size: 10px; font-weight: 900; text-shadow: 0 0 2px rgba(0,0,0,0.5);">+</div>`,
            iconSize: [16, 16],
            iconAnchor: [8, 8]
          }),
          interactive: false
      ...
      // Solo vinculamos eventos si isActive es true
      if (isActive) {
      ...
      // Nota: Se ha eliminado el 'else' con el evento 'click' para evitar la selección desde el mapa.
    }
  }
}
}
const App = () => {
  ...
  const handleSave = useCallback(() => {
    ...
    // 1. Bloqueamos el centrado para esta acción específica
    ...
    // 3. Liberamos el bloqueo inmediatamente después
  ...
  const handleAddNew = useCallback(() => {
    ...
    // Insertar al principio y desactivar centrado automático
      ...
      <Box sx={{
        ...
        <Stack spacing={0.5} sx={{ pl: 1, mt: 0.5 }}>
          {grids.map(g => {
            ...
            const numPaneles = g.paneles?.length || 0;
            const pwr = (numPaneles * (g.config.potenciaW || 0) / 1000).toFixed(2);
            ...
            return (
              <Paper
                ...
                sx={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between', // Asegura que los botones se vayan a los extremos
                  p: 0,
                  borderRadius: 4,
                  cursor: 'pointer',
                  ...
                  width: 'fit-content',
                  minWidth: isSelected ? 160 : 100, // Ajuste dinámico de ancho
                  height: 20,
                  overflow: 'hidden',
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            position: 'fixed',
            // --- COORDENADAS EXACTAS ---
            left: 20, // Tu coordenada X en píxeles
            top: 150,   // Tu coordenada Y en píxeles
            // ---------------------------
            ...
            /* IMPORTANTE: El origen debe ser 0 0 para que coincida con X e Y */
            transform: 'scale(0.75)',
            transformOrigin: '0 0',

            // Aseguramos que no haya transiciones de centrado de Material UI
            margin: 0,
          }
        }}
      >
        {/* Resto de tu contenido (Box, Tabs, etc.) permanece igual */}
              ...
              {innerTab === 0 ? (
                ...
                <Stack spacing={2} alignItems="center">
                  <Box sx={{
                    ...
                    {/* Campo Nombre Grupo (se mantiene del original) */}
                    <TextField
                      label="Nombre Grupo"
                      size="small"
                      variant="standard"
                      ...
                      onChange={(e) => {
                        ...
                        // Solo actualiza si tiene 2 caracteres o menos
