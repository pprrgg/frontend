...
const App = () => {
  ...
  // Función auxiliar para actualizar y mover al principio
  const updateGridAndMoveToTop = useCallback((id, fields) => {
    setGrids(prev => {
      // 1. Encontrar el grid que se está editando
      ...
      const updatedGrid = {
        ...target,
        ...fields,
        config: { ...target.config, ...(fields.config || {}) }
      };

      // 3. Filtrar el array para quitar el viejo y poner el nuevo al principio
      const filtered = prev.filter(g => g.id !== id);
      return [updatedGrid, ...filtered];
    });
  }, []);
  const loadDataFromSession = useCallback((forceCenter = false) => {
    ...
    try {
      ...
      if (parsed["Diseño_FV"]?.length > 1) {
        ...
        // SI LOS DATOS SON NUEVOS O DIFERENTES
        if (dataStr !== lastSavedSnapshot) {
          setGrids(data);
          setLastSavedSnapshot(dataStr);

          // REGLA DE ORO:
          // Si nos piden centrar (forceCenter) Y NO estamos en medio de un Save interno...
          if (forceCenter && !isSavingInternal.current) {
  ...
  const handleSave = useCallback(() => {
    ...
    // 1. Generar el GeoJSON actualizado en tiempo real
    const features = grids.flatMap((grid) => {
      ...
      const m_deg_lat = 111132.9;
      const m_deg_lng = 111412.8 * Math.cos(latRad);

      return grid.paneles.map((panelId) => {
        const [r, c] = panelId.split(',').map(Number);
        const centerOffsetX = c * panelW;
        const centerOffsetY = r * (panelH * Math.cos(tiltRad));

        const coordinates = [[-0.5, -0.5], [0.5, -0.5], [0.5, 0.5], [-0.5, 0.5], [-0.5, -0.5]].map(off => {
          const x = (centerOffsetX + off[0] * panelW) * Math.cos(rad) - (centerOffsetY + off[1] * panelH) * Math.sin(rad);
          const y = (centerOffsetX + off[0] * panelW) * Math.sin(rad) + (centerOffsetY + off[1] * panelH) * Math.cos(rad);
          return [grid.baseLatLng.lng + (x / m_deg_lng), grid.baseLatLng.lat - (y / m_deg_lat)];
        });

        return {
          type: "Feature",
          properties: { area: grid.name, panels: rows * cols, potW: grid.config.potenciaW },
    ...
    // 2. Guardar en sessionStorage incluyendo la nueva columna "geojson"
    const current = JSON.parse(sessionStorage.getItem("excelData") || "{}");

    // Añadimos "geojson" a las cabeceras
    const headers = ["id", "name", "baseLatLng", "rotation", "config", "paneles", "sombras", "geojson"];
    ...
    current["Diseño_FV"] = [
      ...
      ...grids.map(g => [
        g.id,
        g.name,
        JSON.stringify(g.baseLatLng).replace(/"/g, "'"),
        g.rotation,
        JSON.stringify(g.config).replace(/"/g, "'"),
        JSON.stringify(g.paneles).replace(/"/g, "'"),
        JSON.stringify(g.sombras).replace(/"/g, "'"),
        // Guardamos el GeoJSON completo filtrado solo para este grupo si se prefiere, 
        // o el global en cada fila (aquí guardamos el global para fácil acceso)
        JSON.stringify(fullGeoJSON).replace(/"/g, "'")
      ])
    ];
  ...
  const handleAddNew = useCallback(() => {
    ...
    const n = {
      ...
      config: {
        ...
        // --- NUEVAS PROPIEDADES PARA MATRICES ---
        rows: 1,
        cols: 1
        // ---------------------------------------
      },
  ...
  const propsHandlePos = useMemo(() => {
    if (!activeGrid || !mapRef.current) return null;
    const center = mapRef.current.latLngToLayerPoint(activeGrid.baseLatLng);
    const rad = (activeGrid.rotation * Math.PI) / 180;
    const p = L.point(center.x + 85 * Math.cos(-rad), center.y - 85 * Math.sin(-rad));
    return mapRef.current.layerPointToLatLng(p);
  }, [activeGrid]);
  ...
  const exportToGeoJSON = () => {
    ...
    const features = grids.flatMap((grid) => {
      ...
      // Factores de conversión precisos basados en la latitud actual
      const latRad = grid.baseLatLng.lat * (Math.PI / 180);
      const m_per_deg_lat = 111132.954 - 559.822 * Math.cos(2 * latRad);
      const m_per_deg_lng = 111412.84 * Math.cos(latRad);

      return grid.paneles.map((panelId) => {
        const [r, c] = panelId.split(',').map(Number);

        // IMPORTANTE: En Leaflet/Píxeles R positivo es abajo, 
        // pero en Latitud R positivo debe restar grados para ir al Sur.
        // Calculamos el desplazamiento proyectado por la inclinación (tilt)
        const verticalDist = r * (panelH * Math.cos(tiltRad));
        const horizontalDist = c * panelW;

        // Definimos las 4 esquinas relativas al centro de cada BLOQUE (matriz)
        const halfW = panelW / 2;
        const halfH = (panelH * Math.cos(tiltRad)) / 2;

        const cornersOffsets = [
          { dx: -halfW, dy: halfH },  // NW
          { dx: halfW, dy: halfH },   // NE
          { dx: halfW, dy: -halfH },  // SE
          { dx: -halfW, dy: -halfH }, // SW
          { dx: -halfW, dy: halfH }   // Cierre NW
        ];

        const coordinates = cornersOffsets.map(offset => {
          // 1. Posición del panel + posición del vértice dentro del panel
          const totalX = horizontalDist + offset.dx;
          const totalY = verticalDist + offset.dy;

          // 2. Aplicar rotación (Azimut)
          // Usamos -rad porque el sentido de rotación de Leaflet es horario
          const xRot = totalX * Math.cos(rad) - totalY * Math.sin(rad);
          const yRot = totalX * Math.sin(rad) + totalY * Math.cos(rad);

          // 3. Conversión a Grados (Lat/Lng)
          // Restamos yRot de la Latitud porque en ejes cartesianos Y+ es Norte,
          // pero nuestro cálculo de verticalOffset seguía la lógica de pantalla (Y+ es Sur).
          return [
        ...
        return {
          ...
          properties: {
            ...
            bloque: panelId,
            paneles: rows * cols,
            potencia_kWp: ((rows * cols * grid.config.potenciaW) / 1000).toFixed(2)
          },
          geometry: {
            type: "Polygon",
    ...
    a.href = url;
    a.download = `FV_${new Date().getTime()}.geojson`;
    a.click();
  };
  ...
  return (
    <Box sx={{ width: "100%", height: "100vh", position: "relative", overflow: 'hidden' }}>
            ...
            return (
              <Paper
                ...
                sx={{
                  ...
                  width: 'fit-content',
                  minWidth: isSelected ? 160 : 100,
                  height: 20,
                  overflow: 'hidden',
                  transition: 'all 0.3s ease',
                }}
              >
                {/* IZQUIERDA: ACCIONES (Copiar y Config) */}
                {isSelected && (
                  <Stack direction="row" sx={{ height: '100%' }}>
                    <IconButton
                      ...
                      sx={{
                        borderRadius: 0, height: '100%', px: 0.6,
                        color: '#1a237e', bgcolor: 'rgba(255, 255, 255, 0.7)',
                    ...
                    <IconButton
                      ...
                      sx={{
                        borderRadius: 0, height: '100%', px: 0.6,
                        color: '#e65100', bgcolor: 'rgba(255, 255, 255, 0.5)',
                ...
                {/* CENTRO: INFORMACIÓN ACTUALIZADA CON MATRIZ */}
                <Stack direction="row" spacing={0.5} sx={{ px: 1, alignItems: 'center', flexGrow: 1 }}>
                ...
                {/* DERECHA: BORRAR (Solo si está seleccionado) */}
                {isSelected && (
                  <IconButton
                    size="small"
                    onClick={(e) => {
                      e.stopPropagation();
                      setGrids(prev => prev.filter(x => x.id !== g.id));
                      if (isSelected) setActiveGridId(null);
                    }}
                    sx={{
                      borderRadius: 0,
                      height: '100%',
      ...
      >
        ...
        <Tooltip title="Exportar GeoJSON" placement="left">
          <Fab
            size="small"
            ...
            sx={{
              bgcolor: "#1a237e",
              color: "white",
              pointerEvents: "auto", // Habilita clicks
      ...
      >
        <Box sx={{ bgcolor: '#1a237e', color: 'white', p: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="subtitle2" fontWeight={900}>PROPIEDADES</Typography>
          <IconButton size="small" onClick={() => setConfigModalOpen(false)} sx={{ color: 'white' }}><CloseIcon fontSize="small" /></IconButton>
        </Box>

        {activeGrid && (
          <Box sx={{ display: 'flex', flexDirection: 'row', bgcolor: 'white' }}>
            <Box sx={{ borderRight: 1, borderColor: 'divider', bgcolor: '#f9f9f9', width: 45 }}>
              <Tabs orientation="vertical" value={innerTab} onChange={(_, v) => setInnerTab(v)} sx={{ minWidth: 45, '& .MuiTab-root': { minWidth: 45, py: 2 } }}>
              ...
              {innerTab === 0 ? (
                ...
                <Stack spacing={2} alignItems="center">
                  ...
                  <Box sx={{ width: '100%', p: 0.5, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                    ...
                    {/* --- NUEVA SECCIÓN DE MATRIZ (FILAS x COLUMNAS) --- */}
                ...
                /* PESTAÑA 2: MONTAJE (Se mantiene igual) */
                <Stack spacing={2} alignItems="center">
